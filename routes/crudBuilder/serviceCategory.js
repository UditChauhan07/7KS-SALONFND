...
router.get("/", (req, res) => {
  ...
  if (req.session.data) {
    ...
    conn.query(query, (err, result, fields) => {
      ...
      if (result) {
        const getStatus = leftMenuSuppy.get({change:'serviceCat',childEffect:{name:'view'}});
        const chunkSize = 10;
        const newResult = [];
        ...
        res.render("serviceCategory/index", {
          moduleName: name,
...
router.get("/create", (req, res) => {
  ...
  if (req.session.data) {
    ...
    const getStatus = leftMenuSuppy.get({change:'serviceCat',childEffect:{name:'create'}});
    res.render("serviceCategory/create", {
      moduleName: name,
  ...
  if (req.session.data) {
    if (req.session.data.permission.userChild.adminChild.create) {
      if (!table[name]) {
        ...
        const createUser = queries.create(table.serviceCategory, serviceCategoryData);
        conn.query(createUser, (err1, result) => {
          if (err1) {
            console.log(err1);
            req.session.flag = "iURL";
            // res.redirect("/");
          ...
          if (result) {
            req.session.serviceCategoryNewId = result.insertId
            req.session.flag = process.env.ModuleCreateCode;
            res.send({status:true})
          } else {
            // res.redirect("/serviceCategory/create");
            res.send({status:false})
          }
        });
      }
...
router.get("/:id", (req, res) => {
  ...
  if (req.session.data) {
    ...
    const sqDb = queries.oneServiceCategoryIndex(id);
    if (!table[name]) {
      req.session.flag = "iURL";
      res.redirect("/");
      return;
    ...
    conn.query(sqDb, (err, editUser) => {
      ...
      if (editUser.length) {
        ...
        res.render("serviceCategory/edit", {
          moduleName: name,
...
router.post("/:id", (req, res) => {
  ...
  if (req.session.data) {
    if (req.session.data.permission.userChild.adminChild.edit) {
      ...
      const sqDb = queries.edit(table.serviceCategory, editData);
      let htmltable = "user";
      let tStatus = tablestatus.draw(htmltable);
      conn.query(sqDb, (err, result) => {
        ...
        if (result) {
          req.session.serviceCategoryNewId = id;
...
router.get('/:id/delete',(req,res)=>{
  if (req.session.data) {
    if (req.session.data.permission.userChild.adminChild.delete) {
      var id = req.params.id;
      const sqDb = queries.oneServiceCategoryDelete(id);
      if (!table[name]) {
        req.session.flag = "iURL";
        res.redirect("/");
        return;
